...
export function ClaimForm() {
  ...
  useEffect(() => {
    ...
    if (event && mint) {
      try {
        // Validate mint address format - check if the string is valid before creating a PublicKey
        if (mint && typeof mint === 'string' && mint.length > 0) {
          // Only try to create PublicKey with valid string
          new PublicKey(mint);
        ...
        console.error('Error validating mint address:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate we have a non-empty string for the mint address
      if (!mintAddress || typeof mintAddress !== 'string' || mintAddress.trim() === '') {
        throw new Error('Invalid or empty claim code');
      }
      ...
      try {
        ...
        throw new Error('The claim code is not a valid Solana address');
      }
      ...
      try {
        ...
        const response = await fetch('/api/token/claim', {
          ...
          body: JSON.stringify({
            referralCode: mintAddress, // Use the original string to avoid any PublicKey conversion issues
